...
def execute(program):        # Executes program
    ...
    for i,char in enumerate(program):
        ...
        elif char == "-" and not loop: memory[pointer] = 0
        elif char == "=" and not loop: memory[pointer] = 1-memory[pointer]
        elif char == "[":
            if not loop:
                run_loop(program,i)
                loop = True
...
def take_input():
    global memory, pointer, in_memory, in_pointer
    try:
        memory[pointer] = in_memory[in_pointer]
        in_pointer += 1
